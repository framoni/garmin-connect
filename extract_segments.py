...
from pull_activities import email, password, get_runs
...
def extract_segments_from_activity(points: list[dict], segment_len: float, tolerance: float):
    """
    ...
    while i < n:
        ...
        j = i + 1
        while j < n and points[j]['sumDistance'] - points[i]['sumDistance'] < segment_len:
            j += 1
        ...
        if is_constant:
            ...
            cadences = [p['directCadence'] for p in seg_points if p.get('directCadence') is not None]
            hrs = [p['directHeartRate'] for p in seg_points if p.get('directHeartRate') is not None]
            speeds = [p['directSpeed'] for p in seg_points if p.get('directSpeed') is not None and p['directSpeed'] > 0]
...
def main(activities_list: list[dict] | None = None, segment_dist: float = 100.0, tolerance: float = 0.005):
    """
...
def save_to_csv(data: list[tuple], filename: str):
    ...
    header = ['date', 'distance_m', 'incline', 'avg_cadence', 'avg_hr', 'avg_pace_min_km']
...
if __name__ == "__main__":
    ...
    if results:
        print("\n" + "="*85)
        print(f"{'Date':<20} | {'Dist':<7} | {'Incline':<8} | {'Cadence':<8} | {'HR':<6} | {'Pace (min/km)':<12}")
        print("-" * 85)
        for r in results:
            date, dist, incl, cad, hr, pace = r
            print(f"{date[:19]:<20} | {dist:>6.1f}m | {incl:>7.2%} | {cad:>8.1f} | {hr:>6.1f} | {pace:>12.2f}")
